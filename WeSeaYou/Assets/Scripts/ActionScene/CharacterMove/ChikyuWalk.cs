    ...
    void Update()
    {
        ...
        if (IsHandlingPlayer)
        {
            ...
            if (direction == 1)
            {
                ...
                else if (transform.position.x - MyUtyu.transform.position.x >= 0)
                {
                    if (3 >= num0)
                    {
                        playerSpeed.x = moveSpeed;
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                MyUtyu.GetComponent<UtyuWalk>().IsNotHandling_Jump();

            }
        }
        if (!IsHandlingPlayer)
        {
            //playerSpeed.x = Mathf.Sign(MyUtyu.transform.position.x - transform.position.x) * moveSpeed;
            //rb.linearVelocityX = playerSpeed.x;
            //rb.linearVelocityX = moveSpeed * (MyUtyu.transform.position.x - transform.position.x);
            float num1 = Vector3.Distance(transform.position, MyUtyu.transform.position);
