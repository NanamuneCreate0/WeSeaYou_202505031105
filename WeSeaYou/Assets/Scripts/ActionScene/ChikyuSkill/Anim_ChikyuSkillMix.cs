...
public class Anim_ChikyuSkillMix : MonoBehaviour
{
    ...
    IEnumerator SmokeStart(Item item0, Item item1, Item item2)
    {
        ...
        */
        ...
        AnimTable0.GetComponent<Animator>().SetTrigger("Start");//Table動く
        AnimTable1.GetComponent<Animator>().SetTrigger("Start");//Table動く
        yield return new WaitForSeconds(1);
        Smoke.SetActive(true); Smoke.GetComponent<Animator>().SetTrigger("Start");//Smok始まる
        yield return new WaitForSeconds(0.55f);//Smokeが一面を覆う
        AnimTable0.SetActive(false); AnimTable1.SetActive(false);//Table消える
